        ...
        loop {
            {
                ...
                let keys_to_check: Vec<_> = writer_guard.string_db.0.keys().cloned().collect();

                for key in keys_to_check {
                    writer_guard.string_db.check_exist(key);
                }
            }
            {
                // TODO: list
...
async fn test_check_expiration_periodical() {
    ...
    {
        let string_db = &mut db.inner.write().await.string_db;
...
// TODO:
// pub fn gen_rdb_file(db_version ) {
//
// }
